...
void parsefile(FILE *file)
{
    ...
    meta->file = file;
    meta->stack = NULL;
    meta->buf = malloc(sizeof(char) * MAX_LINE_SIZE);  // Add this line

    if (!meta->buf)  // Add this condition
    {
        fprintf(stderr, "Error: malloc failed");
        exit(EXIT_FAILURE);
    }

    while (fgets(meta->buf, MAX_LINE_SIZE, meta->file) != NULL)  // Update this line
    {
        line.number++;
